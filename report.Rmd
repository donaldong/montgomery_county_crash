...
```
<p>
To visualize the crash date time, we created a heatmap. 
Each column in the plot represents a workday. Each row in the plot represents an hour (from 0 - 23).
The brighter the cell is, more crash is recorded at that date time.
Therefore, this plot suggests crashes are more likely to happen in busy hours.
Also it could indicate the different busy hours on weekdays and weekends.
</p>
...
```
<p>
In this plot, we can find the values of latitude(y-value) and longitude(x-values) plotted with a red circle on the map.
</p>
## Surface Condition
```{r}
...
```
<p>
For this feature, we combined the categories with small number of occurrence into 3 categories: `Dry`, `Wet`,
and `Other`. We are interested in this feature because we think surface condition may cause more crashes. 
</p>
## Traffic.Control
```{r}
...
```
<p>
For the traffic control feature, we also keep the top 3 features so it's easy to visualize.
We would be interested in the whether traffic control has any correlations with the location of the crash.
</p>
## Vehicle Body Type
```{r}
...
```
<p>
Since there are many different types of vehicle body, we only keep `PASSENGER CAR`,
which is the majority of the data. We combine the rest into `Other`. We would like to 
find out whether there is any recognizable clusters if we plot the crash locations.
</p>
...
```
<p>
`Vehicle Damage Extent` is another interesting feature. Since there are
`OTHER`,`N/A`, and `UNKNOWN` in the dataset, we combine them into `Other`,
so we can have a better visualization.
</p>
### Vehicle Movement
```{r}
...
```
<p>
Intuitively, we think vehicle movement will be strongly correlated to distribution of crashes.
We keep top five vehicle movement type, and combine the rest into `Other`.
</p>
## Weather
```{r}
...
```
<p>
We think `weather` will also have significant impact on the crashes.
Since there are many different weather types in the dataset, we combined some of
them into `SEVERE`, `CLOUDY`, and `Other`.
</p>
# Combine features
## Bad weather
```{r}
...
```
<p>
Since most of the days have a `CLEAR` weather in Montgomery County,
we removed `CLEAR` and plotted the rest, and look for interesting patterns.
</p>
## Crash Hour Location
```{r}
...
```
<p>
As indicated by previous Crash Date Time plot, crashes are more likely to happen in the busy hours,
which are the blue and purple points in the plot.
</p>
...
```
<p>
We expect Driving Under Influence will have higher fatal injury rate then the others, 
but it does not have that many entries. `None Detected` has the highest ratio of no apparent injury.
</p>
...
```
<p>
By looking at this plot, we think crashes involve multiple drivers seem more likely happen on the main road.
</p>
...
```
<p>
We found there are certain roads have a higher density of the blue dots. 
The plot implies accidents are more likely to happen these roads.
</p>
...
```
<p>
Most big roads have traffic signal, but crash still occurs.
Also there are many dots with red color, which means there isn't any traffic control.
</p>
...
```
<p>
Most vehicles are passenger cars (the dominating blue dots).
</p>
...
```
<p>
Clear weather has a much higher ratio of possible injury.
We think people tend to drive more carefully when the weather is not perfect.
Thus when the weather is good, people won't be that cautious, and more crashes are likely to happen.
</p>
